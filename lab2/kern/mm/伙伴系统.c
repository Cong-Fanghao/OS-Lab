...
best_fit_init_memmap(struct Page *base, size_t n)
{
    ...
    //修改1
    uintptr_t start_addr = page2pa(base);
    size_t remaining = n;

    while (remaining > 0)
    {
        size_t max_order = 0;
        uintptr_t current_addr = page2pa(base);

        while ((1 << (max_order + 1)) * PGSIZE <= remaining * PGSIZE &&
               (current_addr % ((1 << (max_order + 1)) * PGSIZE)) == 0)
        {
            max_order++;
        }

        size_t block_size = 1 << max_order;
...
best_fit_alloc_pages(size_t n)
{
    ...
    if (page != NULL)
    {
        ...
        while (page->property > alloc_size)
        {
            size_t half_size = page->property / 2;
...
best_fit_free_pages(struct Page *base, size_t n)
{
    ...
    do
    {
        ...
        while ((le = list_next(le)) != &free_list)
        {
            ...
            size_t block_size = (1 << order) * PGSIZE;

            uintptr_t base_addr = page2pa(base);
            uintptr_t cand_addr = page2pa(candidate);

            if (cand_addr + block_size == base_addr &&
                candidate->property == base->property)
            {
                uintptr_t buddy_addr = base_addr ^ block_size;
                if (buddy_addr == cand_addr)
                {
                    is_buddy = 1;
                }
            }

            if (!is_buddy && base_addr + block_size == cand_addr &&
            ...
            if (is_buddy)
            {
                struct Page *merge_base = (candidate < base) ? candidate : base;
                merge_base->property *= 2;
                ...
                if (list_empty(&free_list))
                {
                    list_add(&free_list, &(merge_base->page_link));
                }
                else
                {
                    list_entry_t *le = &free_list;
                    while ((le = list_next(le)) != &free_list)
                    {
                        ...
                        if (merge_base < p)
                        {
                            list_add_before(le, &(merge_base->page_link));
                            break;
                        }
                        else if (list_next(le) == &free_list)
                        {
                            list_add(le, &(merge_base->page_link));
                        }
                    }
                }

                SetPageProperty(merge_base);
                base = merge_base; 
                merged = 1;
                break;
            }
